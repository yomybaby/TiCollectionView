...
function createTemplateProject(manifest,cb,isAlloyExample){
	...
	if(isAlloyExample){
		if(!fs.existsSync(path.join(example_project_path,"app"))){
			execSync('alloy new '+example_project_path);
		}
		exec('cp -rf example_alloy/* '+example_project_path+'/app',function(){
			cb();
		});
	}else{
...
gulp.task('ready:iosProject:alloy',['init','ios:build'],function(cb){
...
gulp.task('ready:androidProject:alloy',['init','android:build'],function(cb){
	createTemplateProject(manifests.android,cb,true);
});
...
gulp.task('ios:alloy',['init','ready:iosProject:alloy'],shell.task([
	config.IOS_EXAMPLE_BUILD_COMMAND
],{cwd: example_project_path+'_alloy'}));

gulp.task('android',['init','ready:androidProject'],shell.task([
	config.ANDROID_EXAMPLE_BUILD_COMMAND
],{cwd: example_project_path}));

gulp.task('android:alloy',['init','ready:androidProject:alloy'],shell.task([
	config.ANDROID_EXAMPLE_BUILD_COMMAND
],{cwd: example_project_path+'_alloy'}));
